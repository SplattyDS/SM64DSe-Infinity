...
namespace SM64DSe {
    ...
    public partial class NitroROM {
        ...
        /*private void ChangeParentID(ushort id, ushort parentID, bool dir = true) // dir is file.ID / dir.ID
        {
            if (dir)
            {
                for (int i = 0; i < m_DirEntries.Length; i++)
                {
                    DirEntry dirEntry = m_DirEntries[i];
                    if (m_DirEntries[i].ID == id)
                    {
                        dirEntry.ParentID = parentID;
                        m_DirEntries[i] = dirEntry;
                        return;
                    }
                }
            }
            else // file
            {
                ...
                {
                    FileEntry fileEntry = m_FileEntries[i];
                    if (m_FileEntries[i].ID == id)
                    {
                        fileEntry.ParentID = parentID;
                        m_FileEntries[i] = fileEntry;
                        return;
                    }
                }
            }
        ...
        private void ChangeDirID(string dirName, ushort newID)
        {
            for (int i = 0; i < m_DirEntries.Length; i++)
            {
                DirEntry dirEntry = m_DirEntries[i];
                if (dirEntry.FullName.Equals(dirName))
                {
                    dirEntry.ID = newID;
                    Console.WriteLine("dir id of '" + dirEntry.FullName + "' is now 0x" + Convert.ToString(dirEntry.ID, 16));
                    m_DirEntries[i] = dirEntry;
                    return;
                }
            }
        ...
        private void FixDirectoryOrder() // fixes the order of all directories
        {
            List<DirEntry> dirEntries = m_DirEntries.OrderBy(d => d.FullName).ToList();

            // these use the indexer as the indexer
            bool[] changedDirs = new bool[m_DirEntries.Length];
            bool[] changedFiles = new bool[m_FileEntries.Length];
            for (int i = 0; i < changedDirs.Length; i++) changedDirs[i] = false;
            for (int i = 0; i < changedFiles.Length; i++) changedFiles[i] = false;

            // parent ids
            ushort[] originalDirParentIDs = new ushort[m_DirEntries.Length];
            ushort[] originalFileParentIDs = new ushort[m_FileEntries.Length];
            for (int i = 0; i < originalDirParentIDs.Length; i++) originalDirParentIDs[i] = m_DirEntries[i].ParentID;
            for (int i = 0; i < originalFileParentIDs.Length; i++) originalFileParentIDs[i] = m_FileEntries[i].ParentID;

            for (int i = 0; i < dirEntries.Count; i++)
            {
                ushort ID = (ushort)(0xf000 + i);
                DirEntry curDir = dirEntries[i];

                for (int j = 0; j < changedDirs.Length; j++)
                {
                    if (originalDirParentIDs[j] == curDir.ID)
                    {
                        ChangeParentID(m_DirEntries[j].ID, ID, true);
                        changedDirs[j] = true;
                    }
                }
                ...
                {
                    if (originalFileParentIDs[j] == curDir.ID)
                    {
                        ChangeParentID(m_FileEntries[j].ID, ID, false);
                        changedFiles[j] = true;
                    }
                }

                ChangeDirID(curDir.FullName, ID);
            }

            foreach (DirEntry d in m_DirEntries)
                Console.WriteLine(d.FullName + " (0x" + Convert.ToString(d.ID, 16) + ")");
        }*/
    }
}
