...
namespace SM64DSe
{
    public partial class TextEditorForm : Form
    {
        ...
        public TextEditorForm(string mesgFileName)
        {
            InitializeComponent();
            ReadStrings(mesgFileName);
        }
        ...
        {
            m_ROMFileSelect.ReInitialize("Select a message file to load", new string[] { ".mesg" });
            DialogResult result = m_ROMFileSelect.ShowDialog();

            if (result != DialogResult.OK)
                return;
            
            ReadStrings(m_ROMFileSelect.m_SelectedFile);
        }
