...
namespace SM64DSe
{
    public partial class MainForm : Form
    {
        ...
        private static readonly string[] soundHeaderStart =
        {
            ...
            "};",
            "",
            "consteval SoundInfo GetSoundInfoFromName(const char* name)",
            "{",
        ...
        private static readonly string[] soundHeaderEnd =
        {
            ...
            "",
            "[[gnu::always_inline]]",
            "inline void Play(const char* name, Vector3 camSpacePos)",
            "{",
            "\tSoundInfo info = GetSoundInfoFromName(name);",
            "\tPlay(info.seqArcID, info.seqID, camSpacePos);",
            "}",
            "",
            "[[gnu::always_inline]]",
            "inline void Play2D(const char* name)",
            "{",
            "\tSoundInfo info = GetSoundInfoFromName(name);",
            "\tPlay2D(info.seqArcID, info.seqID);",
            "}",
            "",
            "[[gnu::always_inline]]",
            "inline u32 PlayLong(u32 soundID, const char* name, Vector3 camSpacePos)",
            "{",
            "\tSoundInfo info = GetSoundInfoFromName(name);",
            "\treturn PlayLong(soundID, info.seqArcID, info.seqID, camSpacePos, 0);",
            "}",
            "}",
        };
            ...
            foreach (var l in s)
            {
                ...
                switch (p[0].ToLower())
                {
                    ...
                    case "compile":
                        ...
                        Patcher.PatchProcessor.Process(new DirectoryInfo(basePath), p);
                        break;
                    ...
                    case "replace":
                        ...
                        if (Program.m_ROM.FileExists(p[1] + p[2]))
                        {
                            ...
                            throw new Exception("File not found: " + p[1] + p[2]);
