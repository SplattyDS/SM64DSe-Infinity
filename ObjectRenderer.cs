...
namespace SM64DSe
{
    public class ObjectRenderer
    {
        public static ObjectRenderer FromLevelObject(LevelObject obj)
        {
            ...
            {
                case "NormalBMD":
                    ret = new NormalBMDRenderer(rendererInfo[1], FloatFromString(rendererInfo[2]));
                    break;
                case "NormalKCL":
                    ret = new NormalKCLRenderer(rendererInfo[1], FloatFromString(rendererInfo[2]));
                    break;
                case "DoubleBMD":
                    if (rendererInfo.Length == 4)
                        ret = new DoubleRenderer(rendererInfo[1], rendererInfo[2], FloatFromString(rendererInfo[3]));
                    else
                        ret = new DoubleRenderer(rendererInfo[1], rendererInfo[2], VecFromString(rendererInfo[4]), VecFromString(rendererInfo[5]), FloatFromString(rendererInfo[3]));
                    break;
                case "Kurumajiku":
                    ret = new KurumajikuRenderer(rendererInfo[1], rendererInfo[2], FloatFromString(rendererInfo[3]));
                    break;
                case "Pole":
                    ret = new PoleRenderer(ColorFromString(rendererInfo[1]), ColorFromString(rendererInfo[2]), obj.Parameters[0]);
                    break;
                case "ColorCube":
                    ret = new ColorCubeRenderer(ColorFromString(rendererInfo[1]), ColorFromString(rendererInfo[2]), obj.SupportsRotation());
                    break;
                case "Player":
                    ret = new PlayerRenderer(FloatFromString(rendererInfo[1]), rendererInfo[2]);
                    break;
                case "Luigi":
                    ret = new LuigiRenderer(FloatFromString(rendererInfo[1]));
                    break;
                case "ChainedChomp":
        ...
        private static Vector3 VecFromString(string theString)
        {
            if (theString == "-")
                return Vector3.Zero;

            string[] vals = theString.Split(',');
            if (vals.Length != 3)
                return Vector3.Zero;

            return new Vector3(float.Parse(vals[0], CultureInfo.InvariantCulture.NumberFormat), float.Parse(vals[1], CultureInfo.InvariantCulture.NumberFormat), float.Parse(vals[2], CultureInfo.InvariantCulture.NumberFormat));
        }

        private static float FloatFromString(string theString)
        {
            return float.Parse(theString, CultureInfo.InvariantCulture.NumberFormat);
        }

        private static Color ColorFromString(string theString)
        {
